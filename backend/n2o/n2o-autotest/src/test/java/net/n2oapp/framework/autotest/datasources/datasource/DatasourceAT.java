...
class DatasourceAT extends AutoTestBase {
    ...
    void testFormAsFilter() {
        ...
        Button searchButton = page.regions().region(0, SimpleRegion.class).content()
                .widget(0, FormWidget.class).toolbar().topLeft().button(SEARCH_BUTTON_LABEL);
    ...
    void testValidationManyForm() {
        ...
        validationManyForm(page, "Сергей", dateTimeFormatter.format(LocalDate.now().minusDays(1)), SUCCESS_ALERT_MESSAGE, 1);
        ...
        validationManyForm(page, "Артем", dateTimeFormatter.format(LocalDate.now().minusDays(1)), SUCCESS_ALERT_MESSAGE, 2);
