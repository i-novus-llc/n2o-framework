...
class ChangeDataEventAT extends AutoTestBase {
    ...
    void test() {
        ...
        page.shouldExists();
        page.alerts(Alert.PlaceEnum.TOP).alert(0).shouldNotExists(Duration.ofSeconds(1));
        ...
        text.click();
        text.setValue("test");
        page.alerts(Alert.PlaceEnum.TOP).alert(0).shouldNotExists(Duration.ZERO);

        radioGroup1.check("two");
        page.alerts(Alert.PlaceEnum.TOP).alert(0).shouldHaveColor(ColorsEnum.SUCCESS);
        page.alerts(Alert.PlaceEnum.TOP).alert(0).shouldHaveText("Событие при изменение ds1");

        radioGroup2.check("two");
        page.alerts(Alert.PlaceEnum.TOP).alert(0).shouldHaveColor(ColorsEnum.INFO);
        page.alerts(Alert.PlaceEnum.TOP).alert(0).shouldHaveText("Событие при изменение ds2");
