...
public class SidebarValidator extends TypedMetadataValidator<N2oSidebar> {
    ...
    public void validate(N2oSidebar sidebar, SourceProcessor p) {
        ...
        if (sidebar.getDatasourceId() == null && sidebar.getDatasource() != null) {
            ...
            N2oDatasource datasource = sidebar.getDatasource();
            if (scope.contains(datasource.getId()))
                throw new N2oMetadataValidationException(String.format("Datasource '%s' is already exist", datasource.getId()));
            scope.add(datasource.getId());
        }
