 ...
 */
@Component
public class ObjectElementIOv1 implements NamespaceIO<N2oObject> {
    ...
    private void action(Element e, N2oObject.Operation t, IOProcessor p) {
        ...
        p.read(e, t, (x, y) -> {
            if (y.getValidations() == null) {
                y.setValidations(new N2oObject.Operation.Validations());
                y.getValidations().setActivate(N2oObject.Operation.Validations.Activate.all);
            }
        });
    }
...
//        abstractParameter(e, t, p); todo нельзя использовать из-за конфликта между id и name
    ...
    public String getNamespaceUri() {
        return "http://n2oapp.net/framework/config/schema/object-1.0";
    }
