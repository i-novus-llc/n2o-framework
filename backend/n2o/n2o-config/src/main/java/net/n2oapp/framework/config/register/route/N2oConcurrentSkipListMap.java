 ...
 * @since 1.6
 */
public class N2oConcurrentSkipListMap<K, V> extends AbstractMap<K, V>
    ...
    private V doPut(K key, V value, boolean onlyIfAbsent) {
        ...
        int rnd = N2oThreadLocalRandom.nextSecondarySeed();
     ...
     */
    public N2oConcurrentSkipListMap() {
     ...
     */
    public N2oConcurrentSkipListMap(Comparator<? super K> comparator) {
     ...
     *                              or values are null
     */
    public N2oConcurrentSkipListMap(Map<? extends K, ? extends V> m) {
        this.comparator = null;
        initialize();
        putAll(m);
    }
     ...
     */
    public N2oConcurrentSkipListMap(SortedMap<K, ? extends V> m) {
     ...
     */
    public N2oConcurrentSkipListMap<K, V> clone() {
        try {
            @SuppressWarnings("unchecked")
            N2oConcurrentSkipListMap<K, V> clone =
                    (N2oConcurrentSkipListMap<K, V>) super.clone();
            clone.initialize();
    ...
    abstract class Iter<T> implements Iterator<T> {
        ...
        public void remove() {
            ...
            N2oConcurrentSkipListMap.this.remove(l.key);
            lastReturned = null;
        }
    ...
    static final class KeySet<E>
        ...
        public Iterator<E> iterator() {
            if (m instanceof N2oConcurrentSkipListMap)
                return ((N2oConcurrentSkipListMap<E, Object>) m).keyIterator();
            else
                return ((N2oConcurrentSkipListMap.SubMap<E, Object>) m).keyIterator();
        }
    ...
    static final class Values<E> extends AbstractCollection<E> {
        ...
        public Iterator<E> iterator() {
            if (m instanceof N2oConcurrentSkipListMap)
                return ((N2oConcurrentSkipListMap<?, E>) m).valueIterator();
            else
        ...
        public Spliterator<E> spliterator() {
            if (m instanceof N2oConcurrentSkipListMap)
                return ((N2oConcurrentSkipListMap<?, E>) m).valueSpliterator();
            else
    ...
    static final class EntrySet<K1, V1> extends AbstractSet<Entry<K1, V1>> {
        ...
        public Iterator<Entry<K1, V1>> iterator() {
            if (m instanceof N2oConcurrentSkipListMap)
                return ((N2oConcurrentSkipListMap<K1, V1>) m).entryIterator();
            else
        ...
        public Spliterator<Entry<K1, V1>> spliterator() {
            if (m instanceof N2oConcurrentSkipListMap)
                return ((N2oConcurrentSkipListMap<K1, V1>) m).entrySpliterator();
            else
    ...
    /**
     * Submaps returned by {@link N2oConcurrentSkipListMap} submap operations
    ...
    static final class SubMap<K, V> extends AbstractMap<K, V>
         ...
         * Underlying map
         */
        private final N2oConcurrentSkipListMap<K, V> m;
        /**
         ...
         */
        SubMap(N2oConcurrentSkipListMap<K, V> map,
         ...
         */
        boolean isBeforeEnd(N2oConcurrentSkipListMap.Node<K, V> n,
         ...
         */
        N2oConcurrentSkipListMap.Node<K, V> loNode(Comparator<? super K> cmp) {
            if (lo == null)
                return m.findFirst();
            else if (loInclusive)
         ...
         * most usages need to check bounds.
         */
        N2oConcurrentSkipListMap.Node<K, V> hiNode(Comparator<? super K> cmp) {
            if (hi == null)
                return m.findLast();
            else if (hiInclusive)
        ...
        K lowestKey() {
            ...
            N2oConcurrentSkipListMap.Node<K, V> n = loNode(cmp);
        ...
        K highestKey() {
            Comparator<? super K> cmp = m.comparator;
            N2oConcurrentSkipListMap.Node<K, V> n = hiNode(cmp);
            if (n != null) {
                K last = n.key;
        ...
        public int size() {
            Comparator<? super K> cmp = m.comparator;
            long count = 0;
            for (N2oConcurrentSkipListMap.Node<K, V> n = loNode(cmp);
                 isBeforeEnd(n, cmp);
                 n = n.next) {
                if (n.getValidValue() != null)
                    ++count;
