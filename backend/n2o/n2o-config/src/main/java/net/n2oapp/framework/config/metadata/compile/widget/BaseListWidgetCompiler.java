...
import static net.n2oapp.framework.config.metadata.compile.datasource.DatasourceCompileStaticProcessor.getDatasourceRoute;
...
public abstract class BaseListWidgetCompiler<D extends Widget, S extends N2oAbstractListWidget> extends BaseWidgetCompiler<D, S> {
    ...
    protected Pagination compilePaging(N2oAbstractListWidget source, Integer size, CompileProcessor p, WidgetScope widgetScope) {
        ...
        if (ShowCountTypeEnum.BY_REQUEST.equals(pagination.getShowCount())) {
            addAdditionalMapping(widgetScope, p);
            pagination.setCountDataProvider(initCountDataProvider(widgetScope, p));
        } else if (ShowCountTypeEnum.NEVER.equals(pagination.getShowCount()) && Boolean.FALSE.equals(pagination.getShowLast()))
            addAdditionalMapping(widgetScope, p);
        return pagination;
    }
    ...
    private ClientDataProvider initCountDataProvider(WidgetScope widgetScope, CompileProcessor p) {
        ...
        String route = DatasourceCompileStaticProcessor.getDatasourceRoute(widgetScope.getDatasourceId(), widgetScope.getClientDatasourceId(),null, p);
        countDataProvider.setUrl(p.resolve(property("n2o.api.widget.list.paging.count.url"), String.class) + route);
