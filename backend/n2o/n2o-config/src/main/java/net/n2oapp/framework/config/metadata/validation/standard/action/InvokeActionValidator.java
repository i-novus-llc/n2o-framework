...
public class InvokeActionValidator implements SourceValidator<N2oInvokeAction>, SourceClassAware {
     ...
     */
    private void checkDatasource(N2oInvokeAction source, DataSourcesScope dataSourcesScope) {
     ...
     * @param dataSourcesScope  Скоуп источников данных
     */
    private void checkRefreshDatasources(N2oInvokeAction source, DataSourcesScope dataSourcesScope) {
        if (source.getRefreshDatasources() != null) {
            for (String refreshDs : source.getRefreshDatasources()) {
                String operation = ValidationUtils.getIdOrEmptyString(source.getOperationId());
                ValidationUtils.checkForExistsDatasource(refreshDs, dataSourcesScope,
