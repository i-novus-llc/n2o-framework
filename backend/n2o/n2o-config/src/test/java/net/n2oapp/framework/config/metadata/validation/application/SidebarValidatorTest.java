...
public class SidebarValidatorTest extends SourceValidationTestBase {
    ...
    public void testExisingSidebarInlineDatasource() {
        exception.expect(N2oMetadataValidationException.class);
        exception.expectMessage("Идентификатор 'person' внутреннего источника данных сайдбара уже используется источником данных из скоупа application.xml");
    ...
    public void testUsingInlineDatasourceAndLinkAtTheSameTime() {
        exception.expect(N2oMetadataValidationException.class);
        exception.expectMessage("Сайдбар использует внутренний источник данных и ссылку на источник данных 'ds' из скоупа application.xml одновременно");
