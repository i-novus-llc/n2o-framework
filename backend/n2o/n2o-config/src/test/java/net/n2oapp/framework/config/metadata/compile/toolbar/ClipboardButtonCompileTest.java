...
class ClipboardButtonCompileTest extends SourceCompileTestBase {
    ...
    void test() {
        ...
        StandardField<?> outputText = (StandardField<?>) form.getComponent().getFieldsets().get(0).getRows()
                .get(0).getCols().get(0).getFields().get(0);
        ClipboardButton button = (ClipboardButton) outputText.getToolbar()[0].getButtons().get(0);
        assertThat(button, allOf(
                ...
                hasProperty("iconPosition", is(Position.LEFT)),
                hasProperty("color", is("link")),
                hasProperty("datasource", is("testClipboardButton_ds1")),
                hasProperty("src", is("Buttons/Clipboard"))
        ));
        assertThat(button.getConditions().get(ValidationType.enabled).size(), is(2));
        assertThat(button.getConditions().get(ValidationType.visible).size(), is(1));
        ...
        assertThat(button, allOf(
                ...
                hasProperty("iconPosition", is(Position.RIGHT)),
                hasProperty("color", is("primary")),
                hasProperty("src", is("Buttons/Clipboard"))

        ));
        assertThat(button.getConditions().get(ValidationType.enabled).size(), is(1));
        assertThat(button.getConditions().get(ValidationType.visible), is(nullValue()));
    }
}
