 ...
 * Time: 17:27
 */
public class InOrIsNull_Less extends InListRule {
    @Override
    @SuppressWarnings("unchecked")
    public Filter simplify(Filter left, Filter right) {
        if (right.getType().equals(FilterType.inOrIsNull) && left.getType().equals(FilterType.less))
            return simplify(right, left);
        else if (left.getType().equals(FilterType.inOrIsNull) && right.getType().equals(FilterType.less)) {
    ...
    public Pair<FilterType> getType() {
        return new Pair<>(FilterType.inOrIsNull, FilterType.less);
    }
}
