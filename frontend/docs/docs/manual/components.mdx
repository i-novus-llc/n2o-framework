...
заголовок с навигацией, боковая панель, подвал сайта.
Структура приложения задается в файле `*.application.xml`.
Обычно в приложении существует только один файл `application.xml`
Если их в несколько, то необходимо задать идентификатор структуры приложения в настройке `n2o.application.id`.
...
Подвал сайта - информация, отображаемая внизу страницы.

```xml title="Структура подвала сайта"
<?xml version='1.0' encoding='UTF-8'?>
<application xmlns="http://n2oapp.net/framework/config/schema/application-1.0">
...
Либо перечисляется внутри элементов `<nav>` и `<extra-menu>` хедера или боковой панели:

```xml title="Меню навигации в хедере и боковой панели"
<?xml version='1.0' encoding='UTF-8'?>
<application xmlns="http://n2oapp.net/framework/config/schema/application-1.0">
    <header title="Мое приложение">
        <nav>
...
через атрибут `ref-id` в элементах `<nav>` и `<extra-menu>` хедера или боковой панели:
