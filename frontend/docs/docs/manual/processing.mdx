...
На первом этапе (`Source`) метаданные считываются из исходных файлов (.xml или .groovy) в Java объекты (`N2oMetadata`), [валидируются](#Validating_configurations) и складываются в кэш первого уровня.
