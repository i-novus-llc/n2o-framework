...
В теле `pre-filters` перечисляются [типы фильтрации](#Selection_filters) с указанием поля `field-id` и значения `value`.
...
[моделью](#Widget_models) `resolve` виджета `organizations`.
...
[модели](#Widget_models) виджета `organizations`.
