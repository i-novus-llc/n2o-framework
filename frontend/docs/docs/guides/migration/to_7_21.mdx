...
- Префильтры виджета `<pre-filters>` перенести в элемент `<filters>` внутри `<datasource>`.
...
можно прокинуть на следующую страницу в виде `<datasource>` c атрибутом `default-values-mode`.
Либо на следующей странице указать в `datasource` атрибут `default-values-mode` с соответствующим значением.
...
<show-modal page-id="person" submit-operation-id="create">
    <!-- прокидывание источника с defaults на следующую страницу -->
    <datasources>
...
```xml
<page xmlns="http://n2oapp.net/framework/config/schema/page-4.0">
    <!-- задание default в источнике открываемой страницы -->
    <datasources>
        <datasource id="ds" query-id="data" default-values-mode="defaults"/>
    </datasources>
</page>
```
