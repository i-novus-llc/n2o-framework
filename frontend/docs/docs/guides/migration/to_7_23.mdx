...
- Методы `clickLink` \ `firstTitleShouldHaveText` \ `titleShouldHaveText` \ `titleByIndexShouldHaveText` компонента Breadcrumb объявлены deprecated и перенесены в компонент Crumb.
Обращение к хлебным крошкам имеет теперь следующий вызов `page.breadcrumb.crumb(0)`.
