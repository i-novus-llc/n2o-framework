...
- Свойство `default-values-query-id` у виджетов в `page-3.0` больше не поддерживается,
если необходим такой функционал, следует использовать `default-values-mode="merge"` у `datasource` в `page-4.0`.
