...
**Тело**<br/>
&lt;menu-item&gt; - [Пункт выпадающего меню](#menu-item)<br/>
...
### &lt;clipboard-button&gt; {#clipboard_button}
...
|--------------|-----------|-------------------------------------------------------------------------------------------------|-----------------------|----------------|
| data         | Строка    | Данные для копирования. Возможно использование [плейсхолдеров](../manual/configs/#плейсхолдеры) |                       | !              |
| type         | text html | Тип данных для копирования                                                                      | text                  |                |
| message      | Строка    | Сообщение при успешном копировании                                                              |                       |                |

:::note
Наследует все базовые свойства кнопок, кроме generate.
:::
...
```yaml
# Иконка
n2o.api.clipboard_button.icon = fa fa-copy
# Цвет кнопки
n2o.api.clipboard_button.color = link
# Тип данных для копирования
n2o.api.clipboard_button.type = text
```
...
:::note
Зависимость будет проигнорирована при заданном `disable-on-empty-model=true`
:::

:::note
Для кнопки внутри таблицы, если не задать атрибуты `datasource`/`model`,
то выражение будет считаться относительно текущей строки, а не resolve модели
:::
