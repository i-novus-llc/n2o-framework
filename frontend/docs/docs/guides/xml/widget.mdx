...
&lt;formats&gt; - [Форматы дат и времени](#calendar_formats)<br/>
&lt;dependencies&gt; - [Зависимости виджета](#widget_dependencies)<br/>
&lt;datasource&gt; - [Встроенный источник данных](#widget_datasource)<br/>
...
HTML виджет. Отображает произвольное HTML содержимое с автоматической очисткой через DOMPurify.
По умолчанию удаляются все потенциально опасные теги и атрибуты, включая `<iframe>`, `<script>`, `onclick` и другие.

:::note
Для изменения настроек очистки см. раздел [Настройки безопасности HTML](../manual/security.mdx#настройки-безопасности-html).
:::
...
Через [расширенные свойства](../manual/configs#расширенные-свойства) можно указать дополнительные свойства виджета.
...
|style           |Строка      |Стиль компонента блока                            |                     |
...
**Тело**<br/>
&lt;content&gt; - [Содержимое списка](#list_content)<br/>
...
#Сохранение настроек таблицы в локальное хранилище браузера
n2o.api.widget.table.save_settings=false
```
...
&lt;multi-column&gt; - [Многоуровневый столбец таблицы](#multi-column)<br/>
&lt;dnd-column&gt; - [Drag-n-drop столбец таблицы](#dnd-column)
...
|icon             |[Класс иконки](https://fontawesome.com/v6/icons/)|Иконка заголовка столбца                |                                                            |
...
| Наименование | Тип                                                        | Описание                                                        | Значение по умолчанию                  |
|--------------|------------------------------------------------------------|-----------------------------------------------------------------|----------------------------------------|
| datasource   | Ссылка на [источник данных](../xml/datasource#datasources) | Идентификатор источника данных для вычисления условия видимости | Идентификатор источника данных виджета |
| model        | resolve edit filter multi datasource                       | Модель источника данных для вычисления условия видимости        | filter                                 |
...
<column text-field-id="name">
    <dependencies>
        <visibility datasource="ds" model="resolve">
            type.id == 1
        </visibility>
    </dependencies>
</column>
...
**Тело**<br/>
&lt;cell&gt; - [Ячейка столбца](../xml/cell#cell)<br/>
...
| Наименование | Тип                | Описание                                                                                                                                    | Значение по умолчанию |
|--------------|--------------------|---------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|
| move-mode    | settings table all | Режим перемещения колонок:<br/>`settings` - только через настройки<br/>`table` - только через таблицу<br/>`all` - через настройки и таблицу | all                   |
| fixed        | left right         | Прилипание столбца к краю
...
&lt;column&gt; - [Столбец таблицы](#table_column) <br/>
&lt;filter-column&gt; - [Фильтруемый столбец таблицы](#filter-column)
...
&lt;click&gt; - [Клик по строке](../xml/base#click)<br/>
&lt;overlay&gt; - [Наведение на строку](#overlay)
...
Список [филдсетов](../xml/fieldset)<br/>
...
**Тело**<br/>
&lt;content&gt; - [Содержимое виджета Плитки](#tiles_content)<br/>
...
```yaml
#Количество записей в мульти форме форме (число)
n2o.api.widget.multi_form.size=10
#Показать предупреждение о наличии несохраненных данных в форме
n2o.api.widget.multi_form.unsaved_data_prompt=false
```

**Тело**

&lt;fields&gt; - [Поля формы для каждой строки](#multi_form_fields)<br/>
&lt;pagination&gt; - [Пагинация списка](#pagination)<br/>
&lt;dependencies&gt; - [Зависимости виджета](#widget_dependencies)<br/>
&lt;datasource&gt; - [Встроенный источник данных](#widget_datasource)<br/>
&lt;toolbar&gt; - [Меню управляющих кнопок виджета](#widget_toolbar)<br/>
&lt;actions&gt; - [Встроенные действия](#widget_actions)
...
```xml
<?xml version='1.0' encoding='UTF-8'?>
<multi-form xmlns="http://n2oapp.net/framework/config/schema/widget-5.0"
            datasource="ds1">
    <fields>
        <input-text id="name" label="Имя" text-field-id="name"/>
        <select id="status" label="Статус" text-field-id="status">
            <option value="active" label="Активен"/>
            <option value="inactive" label="Неактивен"/>
        </select>
        <date id="createdDate" label="Дата создания" text-field-id="createdDate"/>
    </fields>
    <datasource>...</datasource>
    <toolbar>...</toolbar>
    <pagination>...</pagination>
...
<Sandbox height={500} projectId="uxcomponents_multi_form"/>
...
**Тело**<br/>
&lt;visibility&gt; - [Условие видимости](../xml/button#visibility)<br/>
&lt;enabling&gt; - [Условие доступности](../xml/button#enabling)
...
[Кнопки настроек таблицы](../xml/tablesettings)

:::note
Кнопки настроек таблицы можно размещать только внутри элементов тулбара таблицы
:::
...
|prev-icon   |[Класс иконки](https://fontawesome.com/v6/icons/)           |Иконка кнопки "Предыдущая страница"                            |fa fa-angle-left                              |
...
|next-icon   |[Класс иконки](https://fontawesome.com/v6/icons/)           |Иконка кнопки "Следующая страница"                             |fa fa-angle-right                             |
