...
|Наименование|Тип                                                                                     |Описание                                                    |Значение по умолчанию                       |
|------------|----------------------------------------------------------------------------------------|------------------------------------------------------------|--------------------------------------------|
|enabled     |Строка                                                                                  |Условие доступности чекбокса                                |Если есть действие, то `true`, иначе `false`|
...
&lt;text-area&gt; - [Компонент ввода многострочного текста](../xml/field#text-area)<br/>
...
| target       | Сценарий открытия ссылки:<br/> `self` - относительный путь добавляется после хоста<br/> `application` - относительный путь добавляется после # | Сценарий открытия ссылки                                    | application           |
...
```xml
<link icon="fa fa-desktop" new-window="true" url="{link}"/>
```
