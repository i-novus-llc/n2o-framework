...
|autocomplete |on off Строка        |Браузерное автозаполнение поля введенными ранее значениями. Списком строковых значений через запятую можно задать [токены](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Attributes/autocomplete#token_list_tokens) для конкретизации подсказок| off        |
...
**Пример**
```xml
<number-picker id="count" min="2" max="10" default-value="5"/>
```
...
| target         | self application | Сценарий открытия ссылки:<br/> `self` - относительный путь добавляется после хоста<br/> `application` - относительный путь добавляется после # | application           |
| new-window     | true false       | Открыть ссылку в новой вкладке                                                                                                                 | false                 |
...
| Наименование | Тип   | Описание                                  | Значение по умолчанию |
|--------------|-------|-------------------------------------------|-----------------------|
| min-rows     | Число | Минимальная высота поля в строках текста  | 3                     |
| max-rows     | Число | Максимальная высота поля в строках текста | 3                     |
| max-length   | Число | Максимальное количество вводимых символов |                       |
| default-value| Строка| Значение по умолчанию

**Настройки**
```yaml
# Максимальная высота поля в строках текста
n2o.api.control.text_area.max_rows = 3
# Минимальная высота поля в строках текста
n2o.api.control.text_area.min_rows = 3
# Минимальная высота поля в строках текста, если <text-area> находится в ячейке <edit>
n2o.api.cell.edit.text_area.min_rows = 1
```
...
## Поля ввода с маской {#masked_fields}

Базовые свойства полей ввода с маской

Наследуются базовые свойства простых компонентов
...
| clear-on-blur | true false | Стирать некорректное значение при смене фокуса                 | true                            |
| default-value | Строка     | Значение по умолчанию
...
# Стирать некорректное значение при смене фокуса
n2o.api.control.masked.clear_on_blur = true
```
...
```yaml
# Браузерное автозаполнение поля введенными ранее значениями
n2o.api.control.masked.masked_input.autocomplete = off
```
...
<Sandbox height={400} projectId="uxcomponents_fields_email" />
...
<Sandbox height={400} projectId="uxcomponents_fields_phone" />
...
<Sandbox height={400} projectId="uxcomponents_fields_snils" />
...
<Sandbox height={400} projectId="uxcomponents_fields_uuid" />
...
| label-field-id       | Поле выборки                                               | Поле выборки, отвечающее за наименование варианта выбора                                           | name                                  |
| search-filter-id     | Поле выборки                                               | Идентификатор фильтра, отвечающего за поиск вариантов выбора при вводе                             | Значение `label-field-id`             |
| sort-field-id        | Поле сортировки                                            | Поле выборки, отвечающее за сортировку вариантов выбора                                            | Значение `label-field-id`             |
...
&lt;default-values&gt; - [Список значений по умолчанию спискового компонента](#default-values)<br/>
...
**Тело**<br/>
&lt;value&gt; - [Значение по умолчанию в списке значений](#value)<br/>
...
<Sandbox height={230} projectId="uxcomponents_select_default_values"/>
...
|input-label-field-id     |Строка       |Идентификатор поля выборки, отвечающего за наименование выбранного варианта  |Значение `label-field-id` |
...
|search-side        |client server|Сторона фильтрации                                                      |server               |
...
# Сторона фильтрации
n2o.api.control.auto_complete.search_side=server
...
# Сторона фильтрации
n2o.api.control.input_select.search_side=server
```
...
# Сторона фильтрации
n2o.api.control.input_select_tree.search_side=server
```
...
:::note
Значение по умолчанию `begin-param` и `end-param` должны быть указаны в формате ISO 8601(yyyy-MM-dd HH:mm:ss или yyyy-MM-dd).
:::
...
**Тело**<br/>
&lt;default-value&gt; - [Значение по умолчанию](#date-interval-default-value)<br/>

#### &lt;default-value&gt; {#date-interval-default-value}
...
**Тело**<br/>
[JavaScript выражение](../manual/javascript)
...
| Наименование | Тип        | Описание                                                                                      | Значение по умолчанию |
|--------------|------------|-----------------------------------------------------------------------------------------------|-----------------------|
| enabled      | true false | Условие срабатывания. Возможно использование [плейсхолдеров](../manual/configs/#плейсхолдеры) | true                  |

**Настройки**

```yaml
# Условие срабатывания
n2o.api.control.dependency.fetch.enabled = true
```
...
```yaml
# Условие срабатывания
n2o.api.control.dependency.fetch_value.enabled = true
```
...
|message-place      |top bottom topLeft topRight bottomLeft bottomRight|Положение сообщения при фиксации                              |                     |
...
|route              |Строка                                            |URL запроса                                                   |Значение `datasource` виджета + '_' + Значение `id` поля|
