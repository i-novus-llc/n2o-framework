...
&lt;filters&gt; - [Предустановленная клиентская фильтрация](../xml/base#pre-filters)

:::note
Клиентская фильтрация поддерживает только фильтрацию `<eq>`
:::
...
|target-field-id   |Строка                                    |Идентификатор поля, в которое производится сохранение             |Идентификатор поля, из которого будут скопированы данные, если не задан target-datasource, иначе - вся модель |
