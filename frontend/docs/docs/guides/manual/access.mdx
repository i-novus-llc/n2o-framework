...
Атрибуты `sec:*` применяются к элементам `<field>`, `<reference>`, `<list>`, `<set>`
в секциях `<in>` и `<out>` операций файла `.object.xml`.
...
Ограничения можно задавать как на родительском элементе (`<reference>`, `<list>`, `<set>`),
так и на конкретных дочерних `<field>`. При этом:

- `sec:*` на `<reference>` / `<list>` / `<set>` скрывает весь объект целиком.
