...
```yaml
#Строгая проверка фильтров доступа. Если true, запрос будет отклонён, когда значение фильтра не удалось вычислить (равно null). Если false, фильтр с неопределённым значением пропускается.
n2o.access.strict_filtering = false
#Применять фильтры <object-filters> к запросам на получение одной записи (size=1). Если false, уникальные запросы обходят фильтрацию доступа.
