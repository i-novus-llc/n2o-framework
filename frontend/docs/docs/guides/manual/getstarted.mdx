...
Для этого необходимо cookie добавить параметр `logLevel` со значением `warn` и перезагрузите страницу

:::tip
Не забудьте отключить её, иначе будет много спама в браузерной консоли, что вызовет лишнее потребление памяти
:::
