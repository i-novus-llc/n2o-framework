...
```
<Sandbox height={100} projectId="interactions_defaultvalue_js"/>
...
```
<Sandbox height={300} projectId="interactions_defaultvalue_datetime"/>
...
На сервере выражения обрабатывает библиотека **GraalJS**, которая поддерживает синтаксис **ECMAScript 2020**.

Доступны все современные возможности JavaScript:
- стрелочные функции
- optional chaining (`?.`)
- nullish coalescing (`??`)
- деструктуризация
