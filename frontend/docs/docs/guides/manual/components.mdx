...
|&lt;multi-form&gt;|MultiFormWidget  |[Мульти-форма](../xml/widget#multi-form)  |
...
Мульти-форма - списковый виджет N2O с расширенными строками. Каждая строка отображается в виде формы (набора полей).

```xml title="Виджет мульти-форма"
<?xml version='1.0' encoding='UTF-8'?>
...
Мульти-форма, как и все списковые виджеты, хранит свои данные в модели `datasource`.
Однако отличием является отсутствие активной строки (записи), а значит и модели `resolve`.

Для работы с данными мульти-формы используется только `datasource` модель!
Если же требуется обратиться к полю конкретной записи, то необходимо использовать префикс `[index].` в имени поля.
Например: `[index].name` для обращения к текущей или `[2].name` для обращения к конкретной строке.

Важно отметить, что во внутренних компонентах мульти-формы нигде не требуется явно указывать `[index]`.
Он автоматически подставится во всех компонентах внутри тега `<fields>`.
Поэтому перейти от обычной формы к мульти-форме в XML можно "бесшовно".

В следующем примере кнопка "Посмотреть описание организации" будет использовать данные из текущей строки (формы).
Кнопка "Добавить" по умолчанию смотрит на модель `datasource`, она создает новый пустой объект в списке.

```xml
<form>
    <multi-form fetch-on-init="true" fetch-on-visibility="true"
                        unsaved-data-prompt="true" auto-focus="true" datasource="ds">
        <fields>
            <row>
                ...
                <input-text id="lastName"
                            label="Фамилия"
                            text-field-id="lastName"/>
                <text id="fullName" label="Полное имя">
                    <dependencies>
                        <set-value on="firstName,lastName">
                            if (firstName &amp;&amp; lastName)
                                return firstName + ' ' + lastName
                        </set-value>
                    </dependencies>
                </text>
                <input-text id="organization"
                            label="Организация"
                            text-field-id="organization.name"/>
            </row>
            <button label="Посмотреть описание организации" enabled="{organization.id != null}">
                <open-page page-id="organization" route="/organization/:organizationId/view">
                    <params>
                        <path-param name="organizationId" value="{organization.id}"/>
                    </params>
                </open-page>
            </button>
        </fields>
