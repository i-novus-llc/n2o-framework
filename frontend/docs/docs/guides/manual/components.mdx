...
**Атрибуты настройки `<columns>`**

`locked` - Список идентификаторов колонок (через запятую), которые отображаются по умолчанию и их нельзя скрыть и передвинуть.<br/>
`default-value` - Список идентификаторов колонок (через запятую), которые отображаются по умолчанию.<br/>
