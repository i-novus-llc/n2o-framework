...
Боковая панель будет отображаться в зависимости от совпадения адреса страницы атрибуту `path` элемента `<sidebar>`.

В случае, если `path` не указан, то он будет отображаться во всем приложении, кроме тех страниц, у которых адрес совпал с атрибутом `path` других боковых панелей.
...
<application xmlns="http://n2oapp.net/framework/config/schema/application-3.0">
    <sidebars>
        <sidebar path="/home" title="Домашняя страница" subtitle="Адрес страницы: my.website.com/home">
          <nav>
            ...
          </nav>
          <extra-menu>
        ...
        <sidebar path="/profile" title="Страница профиля" subtitle="Адрес страницы: my.website.com/profile">
          <nav>
            ...
          </nav>
        </sidebar>
