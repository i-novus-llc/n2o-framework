...
по умолчанию равна true. При переопределении значения данной настройки возможность
...
и в названии страницы разрешаются на клиенте с помощью [источника данных](../xml/page#datasources) страницы, параметры
в `path` — с помощью биндеров на сервере
...
<page xmlns="http://n2oapp.net/framework/config/schema/page-4.0"
    ...
    <breadcrumbs>
        <crumb label="Первая страница" path="/"/>
        <crumb label="Вторая страница" path="/:id/page2"/> <!-- Параметр id разрешается на сервере с помощью биндера -->
...
При этом все используемые параметры должны ссылаться на текущую страницу, в которой компилируется действие открытия
страницы
...
<button label="Изменить"
    ...
    <open-page page-id="page3"
       ...
       <breadcrumbs>
           ...
           <crumb label="Вторая страница" route="/:id/page2"/> <!-- Параметр id разрешается на сервере с помощью биндера -->
...
|&lt;submit&gt;      |n2o/actionImpl/SUBMIT      |[Сохранить источник данных](../xml/action#submit)                                             |
