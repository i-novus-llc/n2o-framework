...
### Репликация маршрутизации с использованием БД
...
#### Репликации роутов через in-memory хеш (redis)

1. Задайте настройку `n2o.config.register.store-type=hash`.
2. Настройте RedisTemplate.
3. Для изменения ключа, по которому будут сохраняться пути, необходимо задать настройку `n2o.config.register.hash.key=routes`.
