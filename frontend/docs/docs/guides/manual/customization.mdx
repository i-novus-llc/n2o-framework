...
Перед тем как подключить кастомизацию к проекту необходимо создать React приложение c N2O Framework (описывается в разделе [Создание проекта](getstarted/)).
...
Проксирование запросов с NodeJs на сервер N2O осуществляется с помощью библиотеки `http-proxy-middleware`.
...
Запуск NodeJs выполняется командой:
...
```
yarn add node-sass@4.14.1 --save-dev
```
