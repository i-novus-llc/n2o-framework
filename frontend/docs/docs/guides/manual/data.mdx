...
```xml title="Использование ключевых слов"
<fields>
    <field id="shortName"/>
    <field id="fullName" normalize="#this != null ? #this : #data['shortName']"/>
</fields>
```
...
|Имя метода               |Описание                                 |Пример вызова                    |Входное значение                   |Выходное значение                  |
|-------------------------|-----------------------------------------|---------------------------------|-----------------------------------|-----------------------------------|
|`jsonToMap`              |Преобразование строки json в объект      |#jsonToMap(#this)                |"{ \"id\": 1,\"name\": \"test\" }" |{ "id": 1, "name": "test" }        |
|`mapToJson`              |Преобразование объекта в строку json     |#mapToJson(#this)                |{ "id": 1, "name": "test" }        |"{ \"id\": 1,\"name\": \"test\" }" |
|`encodeToBase64`         |Преобразование строки из UTF-16 в base64 |#encodeToBase64(#this)           |"test"                             |"dGVzdA=="                         |
|`decodeFromBase64`       |Преобразование строки из base64 в UTF-16 |#decodeFromBase64(#this)         |"dGVzdA=="                         |"test"                             |
|`formatByMask`           |Форматирование значения по маске         |#formatByMask(#this, "##-## ##") |"123456"                           |"12-34 56"                         |
|`formatFullName`         |Форматирование ФИО без сокращений        |#formatFullName(#this)           |["Маркс", "Карл"]                  |"Маркс Карл"                       |
|`formatNameWithInitials` |Форматирование ФИО с инициалами          |#formatNameWithInitials(#this)   |["Толстой", "Лев", "Николаевич"]   |"Толстой Л.Н."                     |

:::note
Для вызова базовых функций в xml укажите ее имя после символа #
:::
...
:::note
Аннотацией @Normalizer также можно пометить класс. В таком случае в контекст будут добавлены
