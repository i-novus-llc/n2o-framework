...
#### Операции над данными {#Operations}
...
<query>
  <list>
    <sql>SELECT :select FROM mytable WHERE :filters ORDER BY :sorting</sql>
  </list>
  <count>
  ...
  <fields>
    <field id="name" select-expression="name"
           sorting-expression="name :direction"/>
  </fields>
</query>
```
...
<query>
  ...
  <filters>
    <eq field-id="name" filter-id="name">{ name: {eq: $$name} }</eq>
  </filters>
...
После маппинга значения, полученные для полей выборки и полей [операции объекта](#Operations) могут быть приведены
...
Для записи выражения нормализации используется атрибут `normalize`

```xml title="Нормаилизация поля выборки"
<fields>
...
В выражениях нормализации могут быть использованы статические java-функции в виде согласно документации
[SpEL](https://docs.spring.io/spring-framework/docs/3.2.x/spring-framework-reference/html/expressions.html)

```xml title="Использование java-функций в выражении нормализации"
<fields>
    <field id="max" normalize="T(java.lang.Math).max(1000, #this)"/>
</fields>
```

##### Добавление собственных нормализаторов

Для собственных функций нормализации в N2O существует механизм их добавления в контекст SpEL. Благодаря этому
появляется возможность не указывать полное имя класса, в котором расположена функция, а ограничиться лишь
ссылкой вида `normalize="#myFunction(#this)"`. Для того чтобы добавить функцию в контекст необходимо сделать её открытой
(public) и указать над ней аннотацию @Normalizer

```java title="Кастомная функция нормализации"
@Normalizer
...
```xml title="Кастомная функция нормализации"
<fields>
...
Аннотацией @Normalize также можно пометить класс. В таком случае в контекст будут добавлены
все открытые статические методы этого класса.

:::

В java-библиотеке N2O уже существует класс `NormalizeUtil`, содержащий базовые функции нормализации, а также
документацию к ним.
...
Также допустим, что имеется некоторая функция нормализации `normalizeOrganization(DataSet org)`, которое преобразует
исходный json к виду:

```json title="Результат преобразования фнукицей normalizeOrganization"
{
    "organization": {
        "codeStr": "2",
        "upperCaseTitle": "ORG2"
...
##### Использование данных других полей

В выражении нормализации одного из полей может быть использовано значение другого поля, находящегося на том же
уровне вложенности. Данный механизм реализуется с использованием SpEL-ссылки `#data`

```xml title="Использование данных других полей"
<fields>
    <field id="email"/>
    <field id="login" normalize="#this ?: #data[email]"/>

    <reference id="dates">
        <field id="from" domain="localdate"/>
        <field id="until" domain="localdate"/>
        <field id="period" normalize="#data[from] + ' - ' + #data[until]"/>
    </reference>
</fields>
```
