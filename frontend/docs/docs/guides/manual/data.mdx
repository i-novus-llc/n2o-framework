...
В java-библиотеке N2O содержится классы `StringNormalizer`, реализующий базовые функции нормализации строковых данных и
`DateFormatNormalizer` для преобразование даты и времени

#### Нормализация строковых данных `StringNormalizer` {#string_normalizer}
...
#### Нормализация даты `DateFormatNormalizer` {#date_format_normalizer}
