...
Адрес GraphQL сервиса, например, `http://localhost:9090/graphql`, можно опускать в элементах `<graphql>`,
...
```
Разделителем между разными фильтрами в параметрах запроса будет `, and: {`, поэтому необходимо задать его атрибутом `filters-separator`.
