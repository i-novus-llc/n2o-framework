...
Стандартные источники данных, как правило, создаются на странице:
...
### Stomp источник данных {#Stomp-Datasources}

Stomp источник данных также получает данные на клиенте, но делает это с помощью STOMP протокола.
Его использование ограничивается *.application.xml файлами.

Пожалуй наиболее частым случаем применения Stomp источника данных является асинхронное получение счетчиков.

```xml title="Изменение счетчика меню с использованием stomp источника данных"
<application xmlns="http://n2oapp.net/framework/config/schema/application-2.0">
    ...
    <header title="Хедер">
        <nav>
            <!-- элемент меню, в котором отобразится обновленное значение счетчика -->
            <!-- атрибут должен ссылаться на имя поля, используемого в stomp-datasource (в данном случае count) -->
            <menu-item id="notif"
...
Изменять можно не только значение элемента меню, но и, например, цвет его значка
```xml title="Изменение цвета значка меню с использованием stomp источника данных"
<application xmlns="http://n2oapp.net/framework/config/schema/application-2.0">
    <datasources>
        <stomp-datasource id="notifCount" destination="/notif">
            <values>
