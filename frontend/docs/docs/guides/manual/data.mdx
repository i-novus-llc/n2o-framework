...
Для кастомизированной обработки ошибок результатов GraphQl сервера требуется переопределить метод `errorHandler` GraphQl провайдера.

```java title="Кастомный обработчик ошибок валидации"
package com.example;
...
public class MyGraphQlDataProviderEngine extends GraphQlDataProviderEngine {
    ...
    protected void errorHandler(DataSet result) {
        ...
        DataList errors = (DataList) result.get(RESPONSE_ERROR_KEY);
        ...
        ...
        throw new N2oUserException("Ошибка валидации", null, validationMessages);
    }
}
```
