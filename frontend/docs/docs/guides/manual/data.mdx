...
Для кастомизированный обработки ошибок результатов GraphQl сервера требуется переопределить метод `errorHandler` GraphQl провайдера.

```java title="Кастомный обработчик ошибок"
package com.example;

public class MyGraphQlDataProviderEngine extends GraphQlDataProviderEngine {
    @Override
    protected void errorHandler(DataSet result) {
        StringBuilder builder = new StringBuilder();
        DataSet errors = result.getDataSet("errors");

        //Получение информации об ошибки валидации
        DataSet validation = errors.getDataSet("validation");
        String variable = validation.getString("variable");
        String value = validation.getString("value");

        //Создание сообщение ошибки
        builder.append("Неверное значение для переменной " + variable + ". ");
        builder.append("Значение не может быть равно " + value + ". ");

        //Выбрасываем исключение
        throw new N2oValidationException(builder.toString());
    }
}
```
