...
1) плейсхолдера `#{context}`, где `context` - это название свойства в [контексте текущего пользователя](../manual/context);
...
где `field` - это ссылка на поле в данных, завязав таким образом [метаданные на данные](../manual/dynamic).
