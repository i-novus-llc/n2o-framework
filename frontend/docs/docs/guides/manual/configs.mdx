...
можно использовать ссылки на данные заданные через плейсхолдер `{expression}`,
где expression - ссылка на поле в текущей модели данных, либо [JavaScript выражение](./javascript).
...
Поведение валидации конфигураций можно настроить с помощью параметра в файле `n2o.properties`:

```properties
# Режим валидации
n2o.validation.mode=on
```
При использовании режима on выполняется полная валидация всех компонентов конфигурации.
При ignore-refs компоненты c заполненным полем ref-id не проходят валидацию.
При использовании режима off валидация полностью отключается.
