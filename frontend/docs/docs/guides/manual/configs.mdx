...
1. плейсхолдер `#{context}`, где `context` - это название свойства в [контексте текущего пользователя](../manual/context)
...
`default-value="{field}"`, завязав таким образом [метаданные на данные](../manual/dynamic);
