...
1) плейсхолдера `#{context}`, где `context` - это название свойства в [контексте текущего пользователя](#Context);
...
где `field` - это ссылка на поле в данных, завязав таким образом [метаданные на данные](#Dynamic_properties).

Тип значения вычисляется автоматически по самому значению, например, `"true"` будет приведен к Boolean.
Чтобы иметь больший контроль над типом и иметь возможность привести значение к строке,
добавьте в начало и конец `'` символ экранирования: `"'true'"` -> `"true"`; `"'123'"` -> `"123"`.
