...
Поведение валидации можно задать с помощью настройки:

```yaml
n2o.validation.mode = on
```
Возможные значения:
- `on` - полная валидация всех компонентов конфигурации
- `ignore-refs` - валидация всех компонентов кроме компонентов с заданным значением `ref-id`
- `off` -  валидация полностью отключается
