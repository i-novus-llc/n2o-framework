...
describe('Проверка api провайдера', () => {
  ...
  it('без параметров', async () => {
    expect.assertions(1);
    return expect(apiProvider()).rejects.toMatch('Неверно задан тип для apiProvider!');
  });
  it('не существующий тип', async () => {
    expect.assertions(1);
    return expect(apiProvider('test')).rejects.toMatch('Неверно задан тип для apiProvider!');
  });
  ...
  it('проверка запроса за метаданными', async () => {
    expect.assertions(1);
    return expect(apiProvider(FETCH_PAGE_METADATA, { pageUrl: 'my/page/test' })).resolves.toEqual({
  ...
  it('проверка запроса за данными', async () => {
    expect.assertions(1);
    return expect(
      apiProvider(FETCH_WIDGET_DATA, { basePath: 'my/data/test', baseQuery: stubOptions })
