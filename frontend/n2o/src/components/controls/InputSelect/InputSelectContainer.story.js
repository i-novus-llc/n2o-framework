...
import { FormFields, FormValidations, excludeMetadata } from 'N2oStorybook/json';
...
const handleData = data => url => {
  ...
  const filterValue = parseUrl(decodeURI(url))[`filter.${InputSelectContainerJson.labelFieldId}`];
  if (filterValue) {
...
stories
  .add(
    ...
    form(() => {
      const props = {
        ...
        imageFieldId: text('imageFieldId', InputSelectContainerJson.imageFieldId),
        multiSelect: boolean('multiSelect', InputSelectContainerJson.multiSelect),
        groupFieldId: text('groupFieldId', InputSelectContainerJson.groupFieldId),
        hasCheckboxes: boolean('hasCheckboxes', InputSelectContainerJson.hasCheckboxes),
  ...
  .add('Кеширование запросов', () => {
    ...
    return (
      <React.Fragment>
        <div className="row">
          <InputSelectContainer {...InputSelectContainerJson} placeholder="Стандартный" />
        </div>
  ...
  .add('Список из метаданных', () => {
    return <InputSelectContainer {...InputSelectContainerJson} options={data} />;
  });
