...
describe('<AdvancedTable/>', () => {
  ...
  describe('тесты rowSelection', () => {
    ...
    it('корректно отрабатывает выбор всех строк', () => {
      ...
      expect(wrapper.find('AdvancedTable').last().state().checkedAll).toBe(true);
      expect(wrapper.find('AdvancedTable').last().state().checked).toEqual({
        '1': true,
        '2': true,
        '3': true,
      });
      ...
      expect(wrapper.find('AdvancedTable').last().state().checkedAll).toBe(false);
      expect(wrapper.find('AdvancedTable').last().state().checked).toEqual({
        '1': false,
        '2': false,
        '3': false,
