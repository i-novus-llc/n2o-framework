...
const setup = propOverrides => {
  const tableData = [
    ...
    { id: '2', name: 'X', surname: 'Y', birthday: '01.01.1001', rowColor: colors[1] },
    { id: '3', name: 'Test', surname: 'Tset', birthday: '01.01.0001', rowColor: colors[2] },
    { id: '4', name: 'Test', surname: 'Tset', birthday: '01.01.0001', rowColor: colors[3] },
    { id: '5', name: 'Test', surname: 'Tset', birthday: '01.01.0001', rowColor: colors[4] }
  ];
  const headers = [
...
describe('Тесты hasSelect и hasFocus', () => {
  ...
  it('проверяет вызывается ли функция установки нового индекса фокуса в state', () => {
    ...
    const stub = sinon.stub(Table.prototype, 'setNewFocusIndex').callsFake(setNewFocusIndex);
    wrapper
      .find('tbody tr')
      .at(0)
      .simulate('click');
  ...
  it('проверяет вызывается ли функция установки нового индекса селекта в state', () => {
    ...
    const stub = sinon.stub(Table.prototype, 'setNewSelectIndex').callsFake(setNewSelectIndex);
    wrapper
      .find('tbody tr')
      .at(0)
      .simulate('click');
  ...
  it('проверяет вызывается ли функция установки новых индексов селекта и фокуса в state', () => {
    ...
    const stub = sinon.stub(Table.prototype, 'setNewSelectIndex').callsFake(setSelectAndFocus);
    wrapper
      .find('tbody tr')
      .at(1)
      .simulate('click');
  ...
  it('проверяет, что при клике TableRow  находится в фокусе при hasFocus', () => {
    ...
    row.simulate('click');
    expect(row.type().toLowerCase()).toEqual(document.activeElement.tagName.toLowerCase());
  });
  ...
  it('проверяет, что при клике TableRow  находится в фокусе при hasFocus == false', () => {
    ...
    const row = wrapper.find('tbody tr').at(1);
    row.simulate('click');
    expect(row.type().toLowerCase() === document.activeElement.tagName.toLowerCase()).toBeFalsy();
  });
});
