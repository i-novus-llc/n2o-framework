...
import { watcherDefaultModels, flowDefaultModels, compareAndResolve } from './pages';
import { channel } from 'redux-saga';
import { select, race, call, take, put, actionChannel } from 'redux-saga/effects';
...
describe('Сага для для наблюдения за изменением модели', () => {
  it('Проверяем watcher дефолтных моделей', () => {
    ...
    expect(gen.next().value).toEqual(race([call(flowDefaultModels, config), take(RESET)]));
  ...
  it('Проверяем flowDefaultModels - только init значение, без подписи', () => {
    ...
    expect(gen.next(state).value).toEqual(call(compareAndResolve, config, state));
  ...
  it('Проверяем flowDefaultModels - observe', () => {
    ...
    expect(gen.next().value).toEqual(actionChannel([SET, COPY, SYNC, REMOVE, UPDATE, UPDATE_MAP]));
  ...
  it('Проверка compareAndResolve если пришел только link и есть что зарезолвить', () => {
    expect(compareAndResolve(resolveModelsLink, { filter: { field: { id: 1 } } })).toEqual({
      resolve: { page_main_create: { name: 1 } }
    });
  });
  it('Проверка compareAndResolve если пришел пришед link и value и есть что зарезолвить', () => {
    expect(compareAndResolve(resolveModelsLinkAndValue, { filter: { field: { id: 1 } } })).toEqual({
      resolve: { page_main_create: { name: 1 } }
    });
  });
});
