...
describe("<Select />", () => {
    ...
    it("проверяет параметр onSelect", () => {
        ...
        wrapper.find("button.n2o-eclipse-content").last().simulate("click");

        wrapper.update();
        expect(wrapper.find("InputAddon").props().item).toEqual(expectedValue);
    });

    it("проверяет reset в Select", () => {
        const { wrapper } = setup();
        wrapper.find("button.n2o-eclipse-content").last().simulate("click");
        wrapper.find(".n2o-input-clear").at(0).simulate("click");
        expect(wrapper.find("SelectComponent").last().state().selected).toEqual([]);
    });
    it("проверяет cleanable в Select", () => {
    ...
    it("проверяет счетчик в placeholder c универсальным заголовком", () => {
        ...
        expect(wrapper.find(".n2o-input-items").contains("Объектов 1 шт")).toEqual(true);
    });
});
