...
import { InputSelectComponent as InputSelect } from "../../inputs/InputSelect/InputSelect";
...
describe("<InputSelect />", () => {
    ...
    it("проверяет параметр labelFieldId", () => {
        ...
        wrapper.find("li").forEach((node, index) => {
            expect(node.children().props().children.slice(-1)[0]).toBe(props.options[index][props.labelFieldId]);
        });
    });
    ...
    it("проверяет параметр valueFieldId", () => {
        ...
        wrapper.find("InputSelect").last().last().setState({ isExpanded: true });
    ...
    it("проверяет параметр disabledValues", () => {
        ...
        wrapper.find("InputSelect").last().setState({ isExpanded: true });
        expect(wrapper.find("button.n2o-eclipse-content").first().props().disabled).toBeTruthy();
    });
    ...
    it("проверяет параметр onSelect", () => {
        ...
        wrapper.find("button.dropdown-item").last().simulate("click");
    ...
    it.skip("добавление объекта при resetOnBlur = false", () => {
        const wrapper = mount(<InputSelect options={props.options} labelFieldId={props.labelFieldId} />);
        wrapper
            .find("input.form-control")
            .simulate("change", { target: { value: "custom name" } })
            .simulate("blur");

        expect(wrapper.find("InputSelect").state().value).toEqual([{ [props.labelFieldId]: "custom name" }]);
    });

    it.skip("проверяет очистку input при resetOnBlur = true", () => {
        const wrapper = mount(<InputSelect options={props.options} labelFieldId={props.labelFieldId} resetOnBlur />);
    ...
    it("проверяет параметр format", () => {
        ...
        wrapper.find("li").forEach((node, index) => {
            ...
            expect(node.children().props().children.slice(-1)[0]).toBe(expected);
        });
    });
    ...
    it("проверяет чекбоксы", () => {
        ...
        wrapper.find("button.dropdown-item").first().simulate("click");
        wrapper.find("InputSelect").last().setState({ isExpanded: true });
        expect(wrapper.find("input.custom-control-input").first().props().checked).toBeTruthy();
    });
    ...
    it("проверяет иконки", () => {
        ...
        wrapper.find("li").forEach((node, index) => {
            expect(node.children().first().props().class).toBe(props.options[index][fieldId]);
        });
    });
    ...
    it("проверяет мульти выбор", () => {
        ...
        expect(wrapper.find("span.selected-item").last().props().title).toBe(props.options[0][props.labelFieldId]);
    });
    ...
    it("проверяет картинку", () => {
        ...
        wrapper.find("InputSelect").last().setState({ isExpanded: true });
        wrapper.find("button.dropdown-item").first().simulate("click");
        expect(wrapper.find(".selected-item img").props().src).toBe(props.options[0][props.imageFieldId]);
    });

    it("Отрисовался status color = success, при statusFieldId", () => {
        const wrapper = mount(<InputSelect {...props4} />);
        wrapper.find("InputSelect").last().setState({ isExpanded: true });
        expect(wrapper.find(".n2o-eclipse-content__with-status").exists()).toBeTruthy();
    ...
    it("Проверяет наличие описания при descriptionFieldId", () => {
        ...
        wrapper.find("InputSelect").last().setState({ isExpanded: true });
        expect(wrapper.find(".n2o-eclipse-content__description").exists()).toBeTruthy();
        expect(wrapper.find(".n2o-eclipse-content__description").first().text()).toBe("Описание");
    });
    it("проверяет disabled элемент при enabledFieldId", () => {
        const wrapper = mount(<InputSelect {...props2} />);
        wrapper.find("InputSelect").last().setState({ isExpanded: true });
        expect(wrapper.find(".n2o-eclipse-content.dropdown-item").hasClass("disabled"));
    });
});
