...
import { appendToArray, removeFromArray, setModel, updateModel } from '../../models/store'
import { type FieldPathOld } from '../../models/Actions'
...
/**
 * Копирование модели по префиксу и ключу в другую модель
 */
...
export const creator = createAction(
    `${MODELS_PREFIX}copy`,
    (payload: Payload, meta: N2OMeta) => ({ payload, meta }),
)

function getAction<T extends Model | Model[]>({ model, newModel, validate, target }: {
    model: T
    newModel: T
    validate?: boolean
    target: FieldPathOld
...
export function* effect({ payload, meta = {}, validate = true }: ReturnType<typeof creator>) {
