...
export function FilterButton(props: Props) {
    ...
    const Button: React.FunctionComponent<Props> | void = getComponent('StandardButton', FactoryLevels.BUTTONS)

    if (!Button) { return null }

    const { label, style, className, onClick, color, noLabelBlock, disabled = false, visible = true } = props
    const button = { label, style, className, disabled, visible, onClick, color }
