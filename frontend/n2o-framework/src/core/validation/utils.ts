...
export const keyToRegexp = (validationKey: string) => new RegExp(`^${validationKey}`.replaceAll(
    INDEX_REGEXP,
    INDEX_MASK,
))
...
export const createRegexpWithContext = (
    ...
    const mask = ctx
        ? str.replaceAll(INDEX_REGEXP, (_, key) => `\\[${ctx[key as keyof ContextType] ?? key}]`)
        : str

    return new RegExp(`^${mask.replaceAll(INDEX_REGEXP, INDEX_MASK)}$`)
}
