...
В N2O компоненты подключаются через конфигурацию **factories** (реестр компонентов/адаптеров).
Есть два основных способа: быстрый (через `createFactoryConfig`) и ручной (тонкая настройка/частичный импорт).
