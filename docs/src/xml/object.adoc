...
<operations>
  <operation
    ...
    <fail-out id="code" mapping="#this.code"/>
    <fail-out id="message" mapping="#this.message"/>
...
Список исходящих link:#_Параметр_провайдера_данных_field[параметров]
ошибки операции <fail-out>.
