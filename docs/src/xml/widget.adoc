...
|cols-sm
|Число
|Количество колонок в мобильной вресии
|2
...
<tiles id="tiles_id" cols-sm="1" cols-md="2" сols-lg="5">
    <content>
     ...
    </content>
</tiles>
...
В качестве содержимого используется список блоков (карточек).
...
Тело::
Блок (карточка). Каждый блок содержит в себе ячейку с данными.

==== <block>
Блок (карточка) плиток.
...
[source,xml]
----
<block id="id2" class="test2" style="test2">
    <cell/>
</block>
----
