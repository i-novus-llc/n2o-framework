...
[source,xml]
----
<!-- определение вложенных полей внутри address.object.xml -->
...
----
[NOTE]
Атрибут entity-class можно указать как в определении составного поля,
так и в объекте, на который указана ссылка. Делается это стандартным образом в теге `<object>`,
как это и показано в примере выше.
...
----
[NOTE]
Описание полей внутри составного поля (reference, list или set) или во внешнем файле — два варианта сделать одно и то же.
Однако, если вы опишите поля в обоих местах, то более приоритетным будет вариант из текущего объекта.
Все поля будут взяты из текущего объекта, а для полей присутствующих в обоих файлах (т.е. с совпадающим id)
...
<object entity-class="com.example.Employee">
  ...
  <fields>
    ...
    <reference id="org" mapping="organisation" object-id="org"/>
...
link:https://n2oapp.net/sandbox/new/data/object/nesting_object_fields_for_map_provider[Пример c Test провайдером]

link:https://n2oapp.net/sandbox/new/data/object/nesting_object_fields_for_args_provider[Пример с Java провайдером]
