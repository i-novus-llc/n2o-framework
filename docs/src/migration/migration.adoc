...
- Добавлен новый функционал для передачи парметров в url открываемой страницы - тэг `<params>`.
- Упразднены `<pre-filters>` в `show-modal` и `open-page`, пока на уровне xml,
в следующих версиях будет убрано совсем. Вместо них необходимо использовать `<params>`. 
